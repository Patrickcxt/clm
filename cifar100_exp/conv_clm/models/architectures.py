...
def mnist_model(inputs,
    ...
    clm_emb = net
    outputs = []
    outputs.append(clm_emb)
    #clm_emb = tf.pad(net, [[0, 0], [4, 4], [4, 4], [0, 0]])
    clm_emb = clm.clm_enc(clm_emb, net_id, 64, stride=1, padding='SAME', is_training=is_training)
    outputs.append(clm_emb)
    clm_emb, shared_emb = clm.clm_shared(clm_emb, 64, padding='SAME', is_training=True)
    outputs.append(shared_emb)
    outputs.append(clm_emb)
    clm_emb = clm.clm_dec(clm_emb, net_id, 64, stride=1, padding='SAME', is_training=is_training)
    outputs.append(clm_emb)
    #net = tf.nn.relu((clm_emb+1.0)*net)
    #net = tf.nn.relu(clm_emb + net)
    net = clm_emb + net
    outputs.append(net)
    print(net)
    ###############################################################
...
def vgg16_model(inputs, emb_size=128, is_training=True, img_shape=None, new_shape=None, dropout_keep_prob=0.5,
    ...
    #net = inputs
    net = tf.image.resize_images(inputs, tf.constant([168, 168]), method=tf.image.ResizeMethod.BILINEAR)
    mean = tf.reduce_mean(net, [1, 2], True)
    std = tf.reduce_mean(tf.square(net - mean), [1, 2], True)
    net = (net - mean) / (std + 1e-5)
    with slim.arg_scope(
        ...
        with slim.arg_scope([slim.dropout], is_training=is_training):
            ...
            with slim.arg_scope([slim.fully_connected], normalizer_fn=None):
                ...
                net = slim.dropout(net, dropout_keep_prob, is_training=is_training,
                                   scope='dropout6')
                emb = slim.fully_connected(net, emb_size, activation_fn=None, scope='fc7')
